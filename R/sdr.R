powerset <- function(set) {
  n <- length(set)
  masks <- 2 ^ (1:n - 1)
  lapply(1:2 ^ n - 1, function(u)
    set[bitwAnd(u, masks) != 0])
}
...
tF <- function(x, ...)
{
  ...
  if ("mu" %in% nx) {
    ...
    if (!is.null(x$mu.initial)) {
      initialize$mu <- function(y, ...) {
        ...
        if (!is.null(bd))
          bd <-
            rep(bd, length.out = if (!is.null(dim(y)))
              nrow(y)
              else
                length(y))
  ...
  if ("sigma" %in% nx) {
    ...
    hess$sigma <- function(y, par, ...) {
      ...
      res <-
        drop(score * sigma.link$mu.eta2(eta) + hess * sigma.link$mu.eta(eta) ^
               2)
      if (!is.null(dim(res)))
        res <- res[, 1]
      res
    ...
    if (link %in% c("logit", "probit", "cauchit", "cloglog", "identity", "log", "sqrt", "1/mu^2", "inverse")) {
      ...
      rval <- switch(
        ...
        "sigmoid" = list(
          ...
          "mu.eta2" = function(eta) {
            eta2 <- eta * 0.5
            - (0.5 * (2 * (sinh(eta2) * 0.5 * cosh(eta2))) / (cosh(eta2) ^
                                                                2) ^ 2)
          }
        )
      )
...
sdr <- function(formula,
    ...
    if (scalex) {
      for (i in names(formula)) {
        ...
        if (length(nn) > 1) {
          ...
          mfd[["coefficients"]][[i]][, range] <-
            t(t(mfd[["coefficients"]][[i]][, range]) * (1/x_sd[range]))
          
          for(j in range){
            mfd[["coefficients"]][[i]][, "(Intercept)"] <- mfd[["coefficients"]][[i]][, "(Intercept)"] -
              mfd[["coefficients"]][[i]][, j] * x_mu[j]
          }  
        }
      }
    }
    ...
    if (exists("outfile") && file.exists(outfile)) {
      unlink(outfile)
    }
    return(mfd)
  }
...
sdr.gradboostfit2_old <-
  function(X,
    ...
    if (is.null(batch_ids)) {
      ind <- 1:N
      b.size <- N
      batch_ids <- lapply(1:maxit, function(...)
        ind)
    } else {
      if (is.numeric(batch_ids)) {
        ind <- 1:N
      ...
      if (!is.null(family$initialize)) {
        ...
        for (j in nxx) {
          if (!is.null(family$initialize[[j]])) {
            linkfun <- make.link2(family$links[j])$linkfun
            beta[[j]][1L, "(Intercept)"] <-
              mean(linkfun(family$initialize[[j]](y,
                                                  ...)), na.rm = TRUE)
    ...
    for (i in 2:maxit) {
      ...
      for (j in nx) {
        ...
        ## Setup linear predictor.
        eta[[j]] <-
          drop(X[[j]][batch_ids[[i]], , drop = FALSE] %*% beta[[j]][i, ])
        
        ## out of sample
        eta.oos[[j]] <-
          drop(X[[j]][batch.oos, , drop = FALSE] %*% beta[[j]][i, ])
      ...
      for (j in nx) {
        ...
        tbeta[1] <- tbeta[1] + err01
        eta[[j]] <-
          drop(X[[j]][batch_ids[[i]], , drop = FALSE] %*% tbeta)
        ll1 <- family$loglik(yi, family$map2par(eta))
      ...
      for (j in nx) {
        ...
        if (nc > 0) {
          ...
          cc <-
            try(cor(grad, X[[j]][batch_ids[[i]], -1, drop = FALSE]), F)
          #[,-1]))
          cc[is.na(cc)] <- 0
          if (is.numeric(cc)) {
            ## Select update
      ...
      for (ij in nx) {
        sign.list[ij] <-
          ifelse(
            i < 0.8 * maxit & sign.list[ij] != 0 & abs(sign.list[ij]) <
              nu * eps[i],
      ...
      ## keep update only if oos information crit improves
      for (j in nx)
        eta.oos[[j]] <- drop(X[[j]][batch.oos, , drop = FALSE] %*%
                               beta[[j]][i, ])
      ll.oos <- family$loglik(y.oos, family$map2par(eta.oos))
      if (K >= 0)
        ic.oos.new <- -2 * ll.oos + K * df
    ...
    for (j in nx) {
      ## Setup linear predictor.
      eta[[j]] <-
        drop(X[[j]][batch_ids[[1]], , drop = FALSE] %*% beta[[j]][maxit, ])
    }
    ## Compute 'out of sample' log-likelihood.
    ll0 <- family$loglik(yi, family$map2par(eta))
    ll0.list <- c(ll0.list, ll0)
...
model.frame.stagewise <- function(formula, data = NULL, ...) {
  dots <- list(...)
  nargs <-
    dots[match(c("data", "na.action", "subset"), names(dots), 0)]
...
model.matrix.stagewise <- function(object, ...) {
  ...
  for (i in names(object$formula)) {
    ...
    tls <-
      unlist(sapply(attr(object$x[[i]]$terms, "specials"), identity))
    if (!is.null(tls)) {
      tll <- tll[-tls]
  ...
  function(object,
    ...
    for (i in 1:length(beta)) {
      if (length(data.frame(beta[i])) != 1)
        df <-
          df + rowSums(ifelse(data.frame(beta[i])[mstart:mstop, ] ==
                                0, 0, 1))
    }
    if (!all) {
      ll <- mean(ll)
...
plot.stagewise_residuals <- function(x,
    ...
    if (length(which) > length(which.match) ||
        !any(which %in% which.match))
      ...
      for (w in which) {
        ...
        if (w == "hist-resid") {
          ...
          if (is.null(args$main))
            args$main <-
            paste("Histogramm and density", if (!is.null(cn[j]))
              paste(":", cn[j])
              else
                NULL)
        ...
        if (w == "qq-resid") {
          if (ncol(x) > 1) {
            ...
            if (is.null(args$main))
              args$main <-
              paste("Normal Q-Q plot", if (!is.null(cn[j]))
                paste(":", cn[j])
                else
                  NULL)
            args$y <- x2[, "Mean"]
            ...
            if (is.null(args$ci.lty))
              args$ci.lty <- 2
            lines(lower$x[order(lower$x)],
                  lower$y[order(lower$x)],
                  lty = args$ci.lty,
                  col = args$ci.col)
            lines(upper$x[order(upper$x)],
                  upper$y[order(upper$x)],
                  lty = args$ci.lty,
                  col = args$ci.col)
            args$y <- x2[, "Mean"]
            qqline(args$y)
        ...
        if (w == "wp") {
          ...
          if (!is.null(xlo)) {
            ...
            if (is.null(args$ci.lty))
              args$ci.lty <- 2
            i <- order(d$xlo)
            lines(d$ylo[i] ~ d$xlo[i],
                  lty = args$ci.lty,
                  col = args$ci.col)
            i <- order(d$xup)
            lines(d$yup[i] ~ d$xup[i],
                  lty = args$ci.lty,
                  col = args$ci.col)
          }
        }
...
rootogram <- function(model,
    ...
    legend(
      ...
      bty = "n",
      legend = c(
        expression(sqrt(pred)),
        expression(sqrt(obs)),
        expression(sqrt(pred) - sqrt(obs))
      ),
      cex = 0.9,
      col = c("blue", "red", "grey"),
      title = NULL,
