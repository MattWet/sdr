...
cs3 = 0.5; cs4 = 0.25; cs5 = -0.25; cs6 = -0.5
...
dgp <- function(nobs = 6000, nnoise = 0, rho = 0, seed = rnorm(1)) {
  ...
  d$y <- rNO(nobs, mu = d$eta.mu, sigma = exp(d$eta.sigma))
  return(d)
}
...
sim <- function(nobs = 500, nnoise = 100, rho = 0){
  ...
  if(TRUE){
    ...
    fam <- NO(mu.link = "identity", sigma.link = "log")
    ...
    fam <- gaussian_bamlss()
    p$sigma <- exp(p$sigma)
    crps <- try(mean(fam$crps(nd$y, p)),T)
    if(!is.numeric(crps)) crps <- NaN
    ll <- fam[["d"]]
    ...
    # combine results
    m <- rbind(
      "likelihood" = c("mu" = mu, "sigma" = sigma, "mse" = mse, "crps" = crps, "logLiK" = l, 
                       "mstop" = maxit, time)
    )
  ...
  if(nobs < 100000){
    ...
    b <- glmboostLSS(f,  families = as.families(NO(mu.link = "identity", sigma.link = "log")),
    ...
    mstop <- mstop(b)[1]
    time <- c(proc.time() - ptm)[3]
    # b is input for var deselection
  ...
  # Var deselection, needs model from Gradient boosting as input. 
  ...
  if(nobs < 100000){
    ...
    timegradboost <- time
    ptm <- proc.time()
    # Gradient boosting with 10 fold cross validation is input for DeselectBoost
    b <- DeselectBoost(b, fam = as.families(NO(mu.link = "identity", sigma.link = "log")), 
                       tau = 0.01)  
    time <- c(proc.time() - ptm)[3] + timegradboost
    ...
    # combine results
    m <- rbind(
      "var_des" = c("mu" = mu, "sigma" = sigma, "mse" = mse, "crps" = crps, "logLiK" = l, 
                    "mstop" = NA, time)
    )
  ...
  if(nobs < 100000){
    ...
    nc <- ncol(d)
    cv_saasl <- cv_risk(d$y, d[,-c(nc-2,nc-1,nc)], folds = folds, method = "SAASL", papply = lapply)
    mstop_saasl <- cv_saasl$mstop
    b <- boost_gaussianLSS(d$y, d[,-c(nc-2,nc-1,nc)], m_stop = mstop_saasl, center_x = TRUE, method = "SAASL")
    time <- c(proc.time() - ptm)[3]
    ...
    ## Predict linear predictors and calculate different metrics on validation data
    p <- list()
    p$mu <- b$mu[1,"(Intercept)"] + as.matrix(nd[,1:(ncol(nd)-3)]) %*% b$mu[,-1]
    p$sigma <- b$sigma[1,"(Intercept)"] + as.matrix(nd[,1:(ncol(nd)-3)]) %*% b$sigma[1,-1]
    ...
    # combine results
    m <- rbind(
      "saasl" = c("mu" = mu, "sigma" = sigma, "mse" = mse, "crps" = crps, "logLiK" = l, "mstop" = mstop_saasl, time)
    )
